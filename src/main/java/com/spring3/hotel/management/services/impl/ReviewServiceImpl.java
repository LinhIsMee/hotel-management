...
public class ReviewServiceImpl implements ReviewService {
    ...
    public List<ReviewResponseDTO> getAllReviews() {
        return convertToDtoList(reviewRepository.findAll());
    }

    @Override
    public Page<ReviewResponseDTO> getAllReviews(Pageable pageable) {
        return convertToPageDto(reviewRepository.findAll(pageable));
    }
    
    @Override
    public Page<ReviewResponseDTO> getPublicReviews(Pageable pageable) {
        return convertToPageDto(reviewRepository.findByStatusNotOrderByCreatedAtDesc(ReviewStatus.HIDDEN, pageable));
    }

    @Override
    public Page<ReviewResponseDTO> getPendingReviews(Pageable pageable) {
        return convertToPageDto(reviewRepository.findByStatus(ReviewStatus.PENDING, pageable));
    }

    @Override
    public Page<ReviewResponseDTO> getRepliedReviews(Pageable pageable) {
        return convertToPageDto(reviewRepository.findByStatus(ReviewStatus.REPLIED, pageable));
    }
    
    @Override
    public Page<ReviewResponseDTO> getHiddenReviews(Pageable pageable) {
        return convertToPageDto(reviewRepository.findByStatus(ReviewStatus.HIDDEN, pageable));
    }
    ...
    public List<ReviewResponseDTO> getReviewsByRoomNumber(String roomNumber) {
        return convertToDtoList(reviewRepository.findByRoomNumber(roomNumber));
    }

    @Override
    public List<ReviewResponseDTO> getReviewsByRoomType(String roomType) {
        return convertToDtoList(reviewRepository.findByRoomType(roomType));
    }
    
    @Override
    public List<ReviewResponseDTO> getFeaturedReviews() {
        return convertToDtoList(reviewRepository.findByIsFeaturedTrue());
    }

    @Override
    public Page<ReviewResponseDTO> getReviewsByMinRating(Integer minRating, Pageable pageable) {
        return convertToPageDto(reviewRepository.findByRatingGreaterThanEqual(minRating, pageable));
    }

    @Override
    public Page<ReviewResponseDTO> searchReviewsByGuestName(String guestName, Pageable pageable) {
        return convertToPageDto(reviewRepository.findByGuestNameContainingIgnoreCase(guestName, pageable));
    }
    ...
    public List<ReviewResponseDTO> getReviewsByRoomId(Integer roomId) {
        return convertToDtoList(reviewRepository.findByRoomId(roomId));
    }
    ...
    // Helper methods để tránh lặp lại code
    ...
    private Page<ReviewResponseDTO> convertToPageDto(Page<Review> reviewsPage) {
        List<ReviewResponseDTO> reviewDTOs = reviewsPage.getContent().stream()
                .map(ReviewResponseDTO::fromEntity)
                .collect(Collectors.toList());
        
        return new PageImpl<>(reviewDTOs, reviewsPage.getPageable(), reviewsPage.getTotalElements());
    }
} 
